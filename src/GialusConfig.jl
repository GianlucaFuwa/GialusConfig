...
macro cn(x)
    if Sys.ARCH === :x86_64
        ...
        esc(
            :(GialusConfig.InteractiveUtils.@code_native syntax = :intel debuginfo =
                :none $x)
        )
    else
        println("julia> @code_native debuginfo=:none ", x)
        esc(:(GialusConfig.InteractiveUtils.@code_native debuginfo = :none $x))
    end
end
macro cl(x)
    println("julia> @code_llvm debuginfo = :none ", x)
    esc(:(GialusConfig.InteractiveUtils.@code_llvm debuginfo = :none $x))
end
macro d(x)
    println("julia> @descend_code_warntype debuginfo = :none ", x)
    esc(
        :(GialusConfig.Cthulhu.@descend_code_typed debuginfo = :none annotate_source =
            false iswarn = true $x)
    )
end
macro bmean(ex)
